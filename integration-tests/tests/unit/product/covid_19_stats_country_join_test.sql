...
{% call dbt_unit_testing.test(ref('covid19_stats')) %}
  {% call dbt_unit_testing.mock_input(ref('covid19_cases_per_day')) %}
     select cast('2021-05-05' as Date) as day, 10 as cases, 'uk' as country_id
  {% endcall %}

  {% call dbt_unit_testing.mock_input(source('dbt_unit_testing', 'covid19_country_stg')) %}
